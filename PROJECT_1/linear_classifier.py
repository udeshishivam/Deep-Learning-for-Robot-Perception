...
def svm_loss_vectorized(
    ...
    loss = torch.sum(margin[(margin>0) & (margin != 1)])/num_train
    loss += reg * torch.sum(W*W)
    ...
    binary = (margin > 0).double()  # (N, C), binary mask where margin > 0
    binary[torch.arange(num_train), y] -= torch.sum(binary, dim=1)

    dW = (X.T @ binary) / num_train  # gradient calculation
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-1, 5e-4, 1e-3, 5e-3, 1e-2]
    regularization_strengths = [1e2, 1e1, 1e0, 1e-1, 1e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    pred_class_train = cls.predict(X_train)
    train_acc = (y_train == pred_class_train).float().mean().cpu()

    pred_class_val = cls.predict(X_val)
    val_acc = (y_val == pred_class_val).float().mean().cpu()
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
      ...
      # print(probs.shape)
      # correct_class_score = scores[y[i]] # one value
...
def softmax_loss_vectorized(
    ...
    correct_class_1hot = torch.zeros_like(probs)
    correct_class_1hot[torch.arange(num_train), y] = 1
    
    dscores = probs - correct_class_1hot
    dW = X.t() @ dscores
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
