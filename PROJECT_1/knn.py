...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    xflat_train = torch.flatten(x_train,start_dim=1)
    xflat_test = torch.flatten(x_test,start_dim=1)

    for i in range(num_train):
      for j in range(num_test):
        dists[i,j] = torch.sum((xflat_train[i, :] - xflat_test[j, :])**2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    xflat_test = torch.flatten(x_test,start_dim=1)
    for i in range(num_train):
      dists[i,:] = torch.sum((xflat_train[i,:] - xflat_test)**2,dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    sq_train = torch.sum(torch.square(xflat_train),dim=1,keepdim=True)
    sq_test = torch.sum(torch.square(xflat_test),dim=1,keepdim=True)
    testXtrain = 2* xflat_train @ xflat_test.t()
    
    dists = sq_train + sq_test.t() - testXtrain
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      # val, idx = torch.topk(nn_arr,k)
      _, close_dist = torch.topk(dists[:,i],k,largest=False)
      knn_arr = y_train[close_dist]
      counts = torch.bincount(knn_arr)
      m = torch.max(counts)
      max_count = torch.where(counts==m)[0]
      y_pred[i] = torch.min(max_count)
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for f in range(num_folds):
        ...
        train_x = torch.cat(x_train_folds[:f] + x_train_folds[f+1:])
        train_y = torch.cat(y_train_folds[:f] + y_train_folds[f+1:])

        knn_cls = KnnClassifier(train_x,train_y)
        acc = knn_cls.check_accuracy(xval,yval, k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accuracies in k_to_accuracies.items():
        ...
        # If we find a better accuracy or a tie with a smaller k
