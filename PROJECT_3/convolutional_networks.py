...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        for height in range(hout):
          for width in range(wout):
            x_fil_region = x_padded[:,:,height*stride:height*stride + HH,width*stride:width*stride + WW]
            x_fil_flat = x_fil_region.reshape(N, -1)#torch.flatten(x_fil_region,1)
            w_flat = w.reshape(F, -1)#torch.flatten(w,1)
            out[:,:,height,width] = x_fil_flat @ w_flat.T + b#.reshape(1,-1)

        # out = out.to('cuda').dtype()

        # print("b",b.reshape(1,-1).shape,"x", (x_fil_flat @ w_flat.T).shape)
    ...
    def backward(dout, cache):
        ...
        # Compute db (bias gradient)
        db = dout.sum(dim=(0, 2, 3))

        # Pad input and gradient w.r.t. input
        x_padded = torch.nn.functional.pad(x, (pad, pad, pad, pad))
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for height in range(hout):
          for width in range(wout):
             out[:,:,height,width]= torch.amax(x[:,:,stride*height:stride*height + pool_height,stride*width:stride*width + pool_width],dim=(2, 3)) 
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
          for c in range(C):
            for height in range(hout):
              for width in range(wout):
                xpool = x[n,c,stride*height:stride*height + pool_height,stride*width:stride*width + pool_width]
                mask = (xpool == torch.max(xpool))
                dx[n,c,stride*height:stride*height + pool_height,stride*width:stride*width + pool_width] = dout[n,c,height,width] * mask
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # Replace "pass" statement with your code
        C, H, W = input_dims
        self.params['W1'] = weight_scale * torch.randn((num_filters, C, filter_size, filter_size), dtype=self.dtype, device=device) 
        self.params['b1'] = torch.zeros(num_filters, dtype=self.dtype, device=device)
        self.params['W2'] = weight_scale * torch.randn((num_filters * (H//2) * (W//2) , hidden_dim), dtype=self.dtype, device=device)
        ...
        # w = torch.nn.init.normal_(w,0,weight_scale)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def loss(self, X, y=None):
        ...
        #Convolution layer
        # out_conv, cache_conv = FastConv.forward(X,W1,b1,conv_param) # out=n,num_filters, hout, wout  cache=x,w,b,conv_param
        # RELU 1
        out_conv, cache_conv = Conv_ReLU_Pool.forward(X,W1,b1,conv_param, pool_param)
        # out_rel1, cache_rel1 = ReLU.forward(out_conv) # out=relu, cache=x
        # # MaxPool 1
        # out_max, cache_max = MaxPool.forward(out_rel1,pool_param)
        # out_max = out_max.reshape(out_max.shape[0], -1)

        #Linear1
        out_lin1, cache_lin1 = Linear_ReLU.forward(out_conv,W2,b2)


        #Linear2
        scores, cache_lin2 = Linear.forward(out_lin1, W3,b3)
        ...
        loss += self.reg * (W1.pow(2).sum() + W2.pow(2).sum() + W3.pow(2).sum())

        dout3, dW3, db3 = Linear.backward(dx, cache_lin2)
        fc,relu = cache_lin1
        dlin1, dW2, db2 = Linear_ReLU.backward(dout3, cache_lin1)

        # dmax = MaxPool.backward(dlin1, cache_max)

        # drel1 = ReLU.backward(dmax, cache_rel1)

        dX,dW1,db1 = Conv_ReLU_Pool.backward(dlin1, cache_conv)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers - 1): # goes from 0 to L-1
          if weight_scale == 'kaiming':
            self.params[f'W{i + 1}'] = kaiming_initializer(num_filters[i], in_filters, K=filter_size, relu=True, dtype=self.dtype, device=device)
          else:
            self.params[f'W{i + 1}'] = torch.randn((num_filters[i], in_filters, filter_size, filter_size),
        ...
        for _ in max_pools:
            pooled_H //= 2
            pooled_W //= 2
        # self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(in_filters * pooled_H * pooled_W, num_classes, dtype=dtype, device=device)
        if weight_scale == "kaiming":
          self.params[f'W{self.num_layers}'] = kaiming_initializer(in_filters * pooled_H * pooled_W, num_classes, relu=True, dtype=self.dtype, device=device)
        else:
          self.params[f'W{self.num_layers}'] = torch.randn((in_filters * pooled_H * pooled_W, num_classes), dtype=self.dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        scores, cache = Linear.forward(out.reshape(X.shape[0], -1), W, b)
        caches.append(cache)
        ...
        #     # if self.batchnorm:
        #     #   bnp_count = i
        #     #   out_cbrp, cache_cbrp[f'cbrp{i+1}'] = Conv_BatchNorm_ReLU_Pool.forward(xin, self.params[f'W{i+1}'], self.params[f'b{i+1}'], \
        #     #                                                                         gamma, beta, conv_param, self.bn_params[bnp_count], pool_param)
        #     #   #cache = (conv_cache, bn_cache, relu_cache, pool_cache)
        #     #   xin = out_cbrp
        #     # else:
        #     out_crp, cache_crp[f'crp{i}'] = Conv_ReLU_Pool.forward(xin,self.params[f'W{i}'], self.params[f'b{i}'],conv_param, pool_param)
        #     xin = out_crp.to(self.dtype)

        #   # elif self.batchnorm:
        #   #   bnp_count = i
        #   #   out_cbr, cache_cbr[f'cbr{i}'] = Conv_BatchNorm_ReLU.forward(xin, self.params[f'W{i}'], self.params[f'b{i}'], \
        #   #                                                                 gamma, beta, conv_param, self.bn_params[bnp_count])
        #   #   xin = out_cbr            
        #   else:
        #     out_cr, cache_cr[f'cr{i}'] = Conv_ReLU.forward(xin, self.params[f'W{i}'], self.params[f'b{i}'], conv_param)
        #     xin = out_cr.to(self.dtype)

        # scores, cache_lin = Linear.forward(xin, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'] )
        ...
        # loss, dout = softmax_loss(scores.to(self.dtype), y)
        # for i in range(1, self.num_layers + 1):
        #     loss += self.reg * torch.sum(self.params[f'W{i}'] ** 2)
        # loss = loss.to(self.dtype)

        # dx, dw, db = Linear.backward(dout.to(self.dtype), cache_lin)
        ...
        #   if (i-1) in self.max_pools:
        #       # if self.batchnorm:
        #       #     dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dx, cache_cbrp[f'cbrp{i}'])
                  
        #       # else:
        #     dx, dw, db = Conv_ReLU_Pool.backward(dx, cache_crp[f'crp{i}'])
        #   # elif self.batchnorm:
        #   #     dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dx, cache_cbr[f'cbr{i}'])
              
        #   else:
        #     dx, dw, db = Conv_ReLU.backward(dx, cache_cr[f'cr{i}'])
        ...
        for i in reversed(range(self.num_layers - 1)):
            if self.batchnorm:
                if i in self.max_pools:
                    dout, grads[f'W{i+1}'], grads[f'b{i+1}'], grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = Conv_BatchNorm_ReLU_Pool.backward(dout, caches.pop())
                else:
                    dout, grads[f'W{i+1}'], grads[f'b{i+1}'], grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = Conv_BatchNorm_ReLU.backward(dout, caches.pop())
            else:
                if i in self.max_pools:
                    dout, grads[f'W{i+1}'], grads[f'b{i+1}'] = Conv_ReLU_Pool.backward(dout, caches.pop())
                else:
                    dout, grads[f'W{i+1}'], grads[f'b{i+1}'] = Conv_ReLU.backward(dout, caches.pop())
...
def find_overfit_parameters():
    weight_scale = 0.9e-3   # Experiment with this!
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # Replace "pass" statement with your code
        fan_in = Din
        weight = torch.randn((Din, Dout), dtype=dtype, device=device) * ((gain / fan_in) ** 0.5)
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = (x,mean,var,x_norm,gamma,beta,bn_param,eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            out = x_norm * gamma + beta
            cache = (x,running_mean,running_var,x_norm,gamma,beta,bn_param,eps)
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x,sample_mean,sample_var,x_normalized,gamma,beta,bn_param,eps = cache
        N, D = x.shape
    ...
    def backward_alt(dout, cache): 
        ...
        dx = (N * dx_normalized - torch.sum(dx_normalized, dim=0) - x_normalized * torch.sum(dx_normalized * x_normalized, dim=0)) / (N * torch.sqrt(sample_var + eps))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        #   x_norm = (x-mean)/(torch.sqrt(var + eps))
        #   # print((x_norm* gamma).shape, " g ", gamma.shape)
        #   out = x_norm * gamma + beta
        #   cache = (x,mean,var,x_norm,gamma,beta,bn_param,eps)
        # elif mode == 'test':
        #   x_norm = (x-running_mean)/(torch.sqrt(running_var + eps))
