...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores -= scores.max(dim=1, keepdim=True)[0]  # Numeric stability trick (subtract max score)
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    hidden_layer = X @ params['W1'] + params['b1'] 
    hidden_layer = hidden_layer.clamp(min=0)
    scores = hidden_layer @ params['W2'] + params['b2']
      ...
      for hidden_size in hidden_sizes:
        for reg in regularization_strengths:
          for decay in learning_rate_decays:
            net = TwoLayerNet(input_size=data_dict['X_train'].shape[1], hidden_size=hidden_size, output_size=len(set(data_dict['y_train'])))
            stats = net.train(data_dict['X_train'], data_dict['y_train'], X_val=data_dict['X_val'], y_val=data_dict['y_val'], 
                        learning_rate=lr, learning_rate_decay=decay, reg=reg, num_iters=1000, batch_size=200)
