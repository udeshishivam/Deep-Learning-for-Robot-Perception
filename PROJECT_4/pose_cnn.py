...
class FeatureExtraction(nn.Module):
    ...
    def forward(self, datadict):
        """
        feature1: [bs, 512, H/8, W/8] #2,512,60,80 --> second arrow corresponding to blue
        feature2: [bs, 512, H/16, W/16] #2,512,30,40 --> first arrow corresponding to purple + blue
        """ 
...
class SegmentationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 64):
        ...
        self.num_classes, H = num_classes, hidden_layer_dim
        in_channel = 512

        self.relu = nn.ReLU(inplace=True)
        self.conv1 = torch.nn.Conv2d(in_channel, H, kernel_size=1, stride=1, padding=0)
        self.conv2 = torch.nn.Conv2d(in_channel, H, kernel_size=1, stride=1, padding=0)

        # self.deconv = nn.ConvTranspose2d(in_channels=H, out_channels=H, kernel_size=4, stride=2, padding=1) 
        
        self.deconv = nn.ConvTranspose2d(H, H, kernel_size=4, stride=2, padding=1)
        
            
        self.conv3 = torch.nn.Conv2d(H, self.num_classes + 1, kernel_size=1, stride=1, padding=0)

        for layer in [self.conv1, self.conv2, self.deconv]:
    ...
    def forward(self, feature1, feature2):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        f1_conv = self.relu(self.conv1(feature1))
        f2_conv = self.relu(self.conv2(feature2))

        f2_upsampled= self.deconv(f2_conv)
        f_add = f2_upsampled + f1_conv
        # print("f_add_conv1",f_add_conv1.shape)
        # f_int_og = torch.nn.functional.interpolate(f_add_conv1, size=feature1.shape[2:], mode='bilinear', align_corners=False)
        f_add_conv1 = self.relu(self.conv3(f_add))
        
        f_int_og = torch.nn.functional.interpolate(f_add_conv1,size=(16 * f2_conv.shape[2],16 * f2_conv.shape[3]), mode='bilinear', align_corners=False)

        probability = torch.nn.functional.softmax(f_int_og,dim=1)
...
class TranslationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 128):
        ...
        self.conv3 = torch.nn.Conv2d(H, 3 * self.num_classes, kernel_size=1, stride=1, padding=0)

        for layer in [self.conv1, self.conv2, self.deconv, self.conv3]:
            nn.init.kaiming_normal_(layer.weight, mode='fan_out', nonlinearity='relu')
            if layer.bias is not None:
                nn.init.zeros_(layer.bias)
    ...
    def forward(self, feature1, feature2):
        ...
        # Replace "pass" statement with your code
        f1_conv = self.relu(self.conv1(feature1))
        # print("f1_conv", f1_conv.shape)
        f2_conv = self.relu(self.conv2(feature2))
        # print("f2_conv", f2_conv.shape)

        f2_upsampled=  self.relu(self.deconv(f2_conv))
        f_add = f2_upsampled + f1_conv
        # print("f_add_conv1",f_add_conv1.shape)
        translation = self.conv3(f_add)
        translation = torch.nn.functional.interpolate(translation,size=(16 * f2_conv.shape[2],16 * f2_conv.shape[3]), mode='bilinear', align_corners=False)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
        return translation
...
class RotationBranch(nn.Module):
    ...
    def __init__(self, feature_dim = 512, roi_shape = 7, hidden_dim = 4096, num_classes = 10):
        ...
        self.roi_pool1 = RoIPool(output_size=(roi_shape, roi_shape), spatial_scale=1/16)
        self.roi_pool2 = RoIPool(output_size=(roi_shape, roi_shape), spatial_scale=1/16)
        ...
        # Kaiming Normal Initialization for weights, bias set to zero
    ...
    def forward(self, feature1, feature2, bbx):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        batch_indices = bbx[:, 0].to(torch.float32)
        rois = torch.cat([batch_indices.unsqueeze(1), bbx[:, 1:5]], dim=1)
        
        # ROI pooling
        # pooled_features = self.roi_pool1(feature2, rois)
        rois_f1 = self.roi_pool1(feature1, rois)
        rois_f2 = self.roi_pool2(feature2, rois)
        pooled_features = rois_f1 + rois_f2
        ...
        # # Sum pooled features
        # rois = rois_f1 + rois_f2
        # rois = torch.flatten(rois, start_dim=1)

        # # Pass through FC layers with LeakyReLU activations
        # fc1 = self.leaky_relu(self.fc1(rois))
        # fc2 = self.leaky_relu(self.fc2(fc1))
        # quaternion = self.fc3(fc2)

        # Reshape and normalize quaternion output
        # quaternion = fc3.view(-1, self.num_classes, 4)
        # # quaternion = torch.nn.functional.normalize(quaternion, dim=2)  
        # quaternion = quaternion.view(-1, 4 * self.num_classes)  
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class PoseCNN(nn.Module):
    ...
    def __init__(self, pretrained_backbone, models_pcd, cam_intrinsic):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.featEx_branch = FeatureExtraction(pretrained_backbone)
        self.seg_branch = SegmentationBranch()
        self.Tx_branch = TranslationBranch()
        self.Rx_branch = RotationBranch()
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            #Segmentation branch
            feature1, feature2 = self.featEx_branch(input_dict)
            probability, segmentation, bbx = self.seg_branch.forward(feature1, feature2)
            loss_dict["loss_segmentation"] = loss_cross_entropy(probability, input_dict['label'])

            #Tx branch
            translation = self.Tx_branch(feature1, feature2)
            # pred_Ts = self.estimateTrans(translation,bbx,segmentation)
            gt_Ts = input_dict['centermaps']
            loss_dict['loss_centermap'] = torch.mean(self.Tx_branch.l1loss(translation, gt_Ts))
            
            #Rx branch
            # IOU selection
            if len(bbx) > 0:
                pred_bbxes = IOUselection(bbx, gt_bbx, self.iou_threshold)
                # print("input_dict['RTs']",input_dict['RTs'].device)

                if len(pred_bbxes) > 0:
                    quaternion = self.Rx_branch(feature1, feature2, pred_bbxes)
                    predRot,label = self.estimateRotation(quaternion, pred_bbxes)
                    gt_Rs = self.gtRotation(pred_bbxes,input_dict).to(device='cuda:0')

                    loss_dict['loss_R'] = torch.mean(loss_Rotation(predRot.to(device='cuda:0'),gt_Rs.to(device='cuda:0'),label,self.models_pcd.to(device='cuda:0')))
            ...
            with torch.no_grad():
                ...
                if len(bbx) > 0:
                    quaternion = self.Rx_branch(feature1, feature2, bbx)
                    pred_Rs, _ = self.estimateRotation(quaternion, bbx)
                else:
                    pred_Rs = torch.zeros((0, 3, 3))
                # print(pred_Rs)
