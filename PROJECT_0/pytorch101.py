...
from torch import Tensor, cpu, cuda, dtype, float64, tensor
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start_multiple = start if start % 10 == 0 else start + (10 - start % 10)
    
    if start_multiple > stop:
      return torch.empty(0, dtype=torch.float64)
    x = torch.arange(start_multiple, stop + 1, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _ , rmin_idx = x.min(dim=1)
    y=x
    y[torch.arange(x.shape[0]),rmin_idx] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    B = x.shape[0]
    z = torch.stack([torch.matmul(x[i], y[i]) for i in range(B)])
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    print(mu)
    sigma = torch.sum((x - mu)**2,dim=0)/(M-1)
...
def mm_on_gpu(x: Tensor, w: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = (torch.matmul(x.to('cuda'),w.to('cuda'))).to('cpu')
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
